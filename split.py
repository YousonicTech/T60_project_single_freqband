...
parser.add_argument("--xls_file", default="/data/pyq/yousonic/aug_wavedata_disturb/gk_yanqihu.xls", type=str)
...
parser.add_argument('--rir_dir', default="/data/pyq/yousonic/aug_wavedata_disturb/", type=str)
parser.add_argument("--output_dir", default="./yanqihu_add_rir", type=str)
parser.add_argument("--gen_convwav_shell", default='./0405/yanqihu_conv_wav.sh', type=str)
...
if __name__ == "__main__":
    ...
    with open(csv_file, 'r') as f:
        ...
        c = 0
        temp = np.arange(min_key, max_key, round(args.split_internal, 1)).tolist()
        temp.append(max_key)
        temp = np.array(temp)
        print(temp)
        # print(np.arange(min_key, max_key, round(args.split_internal, 1)))
        # for key in np.arange(min_key, max_key, round(args.split_internal, 1)):
        for key in temp:
            key = round(key, 1)
            if not key in data_dict.keys():
                print("continue key:", key)
                continue
            # key = round(key,1)
            ...
            for value in values:
                ...
                if not os.path.exists(src_path):
                    print("src_path:%s is not exists" % (src_path))
                    continue
                ...
                shell_str = "python main.py --CORPUS_INPUT_FOLDER_ROOT %s --CORPUS_OUTPUT_FOLDER_ROOT %s --T60DRRresultsFile %s  --need_config %s --MIC_CONFIGs %s\n" % (
                    save_dir, args.output_dir, csv_file, scene_name, scene_name)
